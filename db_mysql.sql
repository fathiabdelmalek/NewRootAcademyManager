...
CREATE TABLE `lessons` (
    ...
    `day of week` ENUM('Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday') NOT NULL,
...
CREATE TABLE `grades` (
    ...
    `year of grade` INTEGER NOT NULL,
    PRIMARY KEY (`id`),
    CHECK (
    	(`level` = 'Primary' AND `year of grade` BETWEEN 1 AND 5) OR
    	(`level` = 'Middle' AND `year of grade` BETWEEN 1 AND 4) OR
    	(`level` = 'Secondary' AND `year of grade` BETWEEN 1 AND 3)
    )
);
...
SELECT `students`.`id`, `students`.`first name`, `students`.`last name`, `students`.`phone number`, `students`.`sex`, `students`.`birth date`, CONCAT(`grades`.`year of grade`, ' ', `grades`.`level`) AS `grade`
FROM `students`
INNER JOIN `grades` ON `grades`.`id` = `students`.`grade id`;

CREATE OR REPLACE VIEW `lessons view` AS
SELECT `lessons`.`id`, `lessons`.`lesson name`, `lessons`.`price`, `lessons`.`classes number`, `lessons`.`day of week`, `lessons`.`start time`, `lessons`.`end time`, CONCAT(`teachers`.`first name`, ' ', `teachers`.`last name`) AS `teacher name`, `rooms`.`code` AS `room code`, CONCAT(`grades`.`year of grade`, ' ', `grades`.`level`) AS `grade`, ( SELECT COUNT(*) FROM `attendances` WHERE `attendances`.`lesson id` = `lessons`.`id` ) AS `students number`
FROM `lessons`
